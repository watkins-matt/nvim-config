...
def check_update():
    ...
    if current_version != latest_version or not is_nvim_installed_correctly():
        logging.info(
            f"New version found: {latest_version}. Current version: {current_version}"
        )
        download_nvim()
        with open(version_file_path, "w") as file:
...
def create_update_alias():
    ...
    if alias_added:
        ...
        try:
            ...
            logging.info(
                "Please run the following command to use the new alias in the current session:"
            )
    ...
    logging.info(
        "For the current session, you may need to run the following command if the alias is not working:"
    )
    logging.info(f"    {reload_command}")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        logging.info(
            "Neovim has been installed/updated. You can now use the 'nvim' command."
        )
        ...
        logging.info(
            "The 'update-nvim-config' alias has been created for instant config updates."
        )
        logging.info(
            "If the alias doesn't work immediately, please reload your shell configuration."
        )
